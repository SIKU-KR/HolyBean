...
class MenuRepository @Inject constructor(
    ...
    suspend fun saveMenuOrders(items: List<MenuItem>) = withContext(ioDispatcher) {
    ...
    suspend fun updateSpecificMenu(item: MenuItem) = withContext(ioDispatcher) {
        val current = getMenuListSync().associateBy { it.id }.toMutableMap()
        current[item.id] = item
        writeAll(current.values.sortedBy { it.id })
    }

    suspend fun addMenu(item: MenuItem) = withContext(ioDispatcher) {
        val current = getMenuListSync().filter { it.id != item.id }
        writeAll((current + item).sortedBy { it.id })
    }

    suspend fun isValidMenuName(newName: String): Boolean = withContext(ioDispatcher) {
        getMenuListSync().none { it.name == newName }
    }

    suspend fun getNextAvailableIdForCategory(category: Int): Int = withContext(ioDispatcher) {
        nextAvailable(getMenuListSync().map { it.id }, category)
    }

    suspend fun getNextAvailablePlacementForCategory(category: Int): Int = withContext(ioDispatcher) {
